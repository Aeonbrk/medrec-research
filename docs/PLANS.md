...
- **Scientific Findings**:
  ...
  - Semantic B (strict alternative admission) failed materiality: only 1 relation (`C09A -> C09C` in `C09`) met Tier-A guideline criteria at the ATC-3 class resolution. Re-audit of antibacterial candidate relations (`J01C -> J01D`, `J01D <-> J01M`) under frozen protocol §8.1(4) confirmed that guidelines support only narrow, agent-specific regimens rather than wholesale class interchangeability between heterogeneous ATC-3 groups. In total, 22 relations were rejected due to complementary combinations (multimodal analgesia `N02B <-> N02A`, sequential nephron blockade `C03C -> C03A`), disjoint severity stages (`A02B <-> A02A`), non-substitutable contraindications (`C08C -> C08D`), or failure of class-level equivalence.
  - Admitted relations cover only 11 distinct patients (2.79% < 25.0%) and span only 1 qualifying ATC-2 parent (`C09` with 11 patients; required $\ge 3$ parents each with $\ge 10$ patients).
  - Strong negative control `NAIVE_SHARED_INDICATION`: among the 14 supported relations labeled `NAIVE_SHARED_INDICATION`, 13 (92.9%) failed strict semantic admission, demonstrating that shared indication $\neq$ therapeutic substitution.
