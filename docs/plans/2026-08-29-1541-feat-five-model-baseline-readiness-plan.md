...
## Execution Outcome: 2026-08-31

- U1–U2 completed without retraining. The seven immutable recovered training pairs and exact frozen schedule were reopened through an additive continuation. Validation-only selection remained `molerec-safedrug-lr-5e-4`; the other two SafeDrug candidates remain `not_tested_by_design`.
- U3 completed through `continuation-20260830-pathfix-1`. RETAIN, LEAP, GAMENet, selected SafeDrug, and MoleRec each produced one legal finalized upstream ten-round test pair. The original failed RETAIN queue remains immutable and contributed no metric.
- The Phase A axes are `execution_integrity = passed`, `paper_point_fidelity = failed` (16/25), `directional_relationships = failed` (3/4), and `artifact_completeness = passed`. The verdict is `completed_mismatch`.
- U4–U6 completed at clean Comparison revision `9fa239269f5a9ac0c394263ebe0ba3c02fbdafc5`: one v1.1 protocol packet, five method profiles, two static target-free adapters, a core-owned target join, five outcomes, deterministic uncertainty, and seven ordered qualification gates passed local conformance.
- U7 completed on 319. All five methods passed `environment_lock`, `adapter_smoke`, `cohort_identity`, `adaptation_budget`, `core_integrity`, `deterministic_adapter`, and `independent_evaluation` under one shared Comparison Scope. SafeDrug and MoleRec used the explicit user-authorized shared-GPU Phase B exception after GPU 0 showed 0% utilization and sufficient free memory; the resident external process was not stopped.
- U8 completed. The five target baselines are `comparison_ready`, the built-in reference remains `registered`, public-safe evidence contains no patient record, prediction batch, checkpoint, weight, private path, hostname, or raw trace, and the three suite conclusions are all true.
