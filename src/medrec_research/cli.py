...
def _build_parser() -> argparse.ArgumentParser:
    ...
    action.add_argument("--intent", type=Path, required=True)
...
def _status_publish(arguments: argparse.Namespace, clock: Clock) -> tuple[int, str | None]:
    ...
    if review is not None and state.review_state is not HumanReviewState.ACCEPTED:
        raise ProtocolValidationError("explicit human review is stale")
    characterization = (
...
def _action_evaluate(arguments: argparse.Namespace, clock: Clock) -> tuple[int, str | None]:
    intent = ActionIntent.from_dict(
        parse_json_object(arguments.intent.read_text(encoding="utf-8"), context="ActionIntent")
    )
...
def _serve_harness(arguments: argparse.Namespace, clock: Clock) -> tuple[int, str | None]:
    ...
    expected = bundle.current_authorities if bundle is not None else snapshot.authorities
    server = create_harness_server(
        status_path=arguments.status,
        expected_authorities=expected,
        clock=clock,
        port=arguments.port,
        actions_enabled=bundle is not None,
        authority_bundle=bundle,
    )
