#!/usr/bin/env python3
"""Stage Idea 005 validation-only inputs on 319.

Runs in the frozen MoleRec baseline environment. It indexes only the training-free
validation staging path needed for Gate 01; the test patient range is never iterated.
"""
...
def _vocabulary(idx2word: object) -> tuple[str, ...]:
    return tuple(str(idx2word[index]) for index in range(len(idx2word)))  # type: ignore[arg-type,index]
...
def stage_gate01_inputs(
    ...
    validation_patient_indices = _split_ranges(len(records))["validation"]
    ...
    for patient_order, patient_index in enumerate(validation_patient_indices):
        ...
        for visit_order, visit_index in enumerate(range(1, len(patient)), start=1):
            ...
            targets[visit_key] = [medication_vocabulary[int(code)] for code in current[2]]
            traversal.append(
                {
                    "patient_id": patient_id,
                    "visit_id": visit_id,
    ...
    meta = {
        ...
        "visit_traversal_metadata": traversal,
        "snapshot_sha256": snapshot_sha256,
        "ddi_asset_sha256": ddi_asset_sha256,
        "feature_availability_sha256": _content_sha256(FEATURE_RULE),
        "medication_vocabulary_sha256": _vocabulary_sha256(medication_vocabulary),
...
def main() -> None:
    ...
    meta = stage_gate01_inputs(args.dataset_root, args.output_dir, args.dataset_id)
    print(
        json.dumps(
            {
