#!/usr/bin/env python3
"""Idea 003 validation staging and train-only prevalence extraction.
...
Accesses only the training split (for prevalence) and validation split (for staging).
...
def stage_gate01_inputs(
    ...
    # 3. Splits: strictly index train (prevalence) and validation (staging). Test is NEVER touched.
    splits = _split_ranges(len(records))
    train_patient_indices = splits["train"]
    validation_patient_indices = splits["validation"]

    # 4. Train-only prevalence calculation
    ...
    # Laplace-smoothed train-only prevalence: p_train(m) = (C_train(m) + 1) / (V_train + 2)
    p_train: dict[str, float] = {}
    for med in medication_vocabulary:
        count = c_train.get(med, 0)
        p_train[med] = float((count + 1) / (v_train + 2))

    # Save restricted train-only prevalence table
    train_prev_path = output_dir / "train_prevalence.json"
    train_prev_path.write_text(
        json.dumps(
            {
    ...
    meta = {
        ...
        "train_prevalence_path": str(train_prev_path),
        "eligible_train_visits": v_train,
        ...
        json.dumps(
            {
                ...
                "train_prevalence_path": meta["train_prevalence_path"],
