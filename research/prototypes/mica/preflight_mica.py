...
from mica import MICA, objective, pack_inputs
...
def run(device_name: str = "cpu") -> dict:
    ...
    if list(early.state_dict()) != list(late.state_dict()) or any(not torch.equal(a, b) for a, b in zip(early.state_dict().values(), late.state_dict().values())) or sum(p.numel() for p in early.parameters()) != sum(p.numel() for p in late.parameters()):
    ...
    early.eval(); late.eval()
    with torch.no_grad():
        e0 = early(packed); l0 = late(packed)
    ...
    gradients = {}
    target = torch.zeros((2, 131), device=device); target[0, 0] = 1.0
    ddi = torch.zeros((131, 131), device=device)
    ddi[0, 1] = ddi[1, 0] = 1.0
    for name, model in (("early", early), ("late", late)):
        model.train(); model.zero_grad(set_to_none=True)
        loss, _, _ = objective(model(packed), target, ddi); loss.backward()
        gradients_ok = all(p.grad is not None and torch.isfinite(p.grad).all() for p in model.parameters())
        ...
        gradients[name] = norm
    return {"status": "PASS", "device": str(device), "parameter_count": sum(p.numel() for p in early.parameters()), "conditioner_gradient_l1": gradients, "target_free_interface": True}


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--device", choices=("cpu", "cuda"), default="cpu"); args = parser.parse_args()
