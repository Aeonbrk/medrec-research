#!/usr/bin/env python3
"""Run one 30-epoch lane of the medication-evidence mutual-binding screen."""
...
from memb_model import (  # noqa: E402
    ALL_VARIANTS,
    MEMB_VARIANTS,
    MEMBModel,
...
def _make_model(diagnosis_count: int, procedure_count: int, variant: str) -> torch.nn.Module:
    if variant in MEMB_VARIANTS:
        return MEMBModel(diagnosis_count, procedure_count, variant)
    ...
    raise RuntimeError("unknown MEMB screen variant: " + variant)
...
def _config(lane: str, variant: str, numeric_policy: Mapping[str, Any], epochs: int) -> Dict[str, Any]:
    return {
        "profile_id": PROFILE_ID,
        "architecture_family": "MEMB",
        "lane": lane,
        "variant": variant,
        "epochs": epochs,
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    if args.lane not in LANES:
        raise RuntimeError("unknown MEMB lane")
    variant = LANES[args.lane]
    if variant not in ALL_VARIANTS:
        raise RuntimeError("lane maps to unknown MEMB screen variant")
    ...
    if epochs not in {30, 60}:
        raise RuntimeError("MEMB supports only 30 epochs or exact 60-epoch censoring extension")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the full MEMB screen")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, len(order), BATCH_SIZE):
            ...
            batch = {
                key: value.to(device)
                for key, value in pack_rows(
                    _model_rows([train_rows[int(index)] for index in indices]), dx_count, proc_count
                ).items()
            }
        ...
        selected_metrics = next(
            item["metrics"] for item in candidate_results
    ...
    horizon_censored = epochs == DEFAULT_EPOCHS and best_candidate.checkpoint > HORIZON_SAFE_MAX_EPOCH
