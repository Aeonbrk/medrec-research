#!/usr/bin/env python3
"""Run one frozen survivor-discrimination lane."""
...
from research.prototypes.paper_contract.evaluator import SelectionCandidate, select_joint  # noqa: E402
from survivor_model import (  # noqa: E402
    MEDICATIONS,
    SurvivorModel,
...
SEED_CONDITIONS: Dict[str, Dict[str, int]] = {
    "canonical": {"torch": 1203, "cuda_torch": 1203, "python_random": 1203, "numpy": 2048},
    "stability_1": {"torch": 1204, "cuda_torch": 1204, "python_random": 1204, "numpy": 2049},
    "stability_2": {"torch": 1205, "cuda_torch": 1205, "python_random": 1205, "numpy": 2050},
    "stability_3": {"torch": 1206, "cuda_torch": 1206, "python_random": 1206, "numpy": 2051},
}

LANES: Dict[str, Dict[str, str]] = {
    "resolution_visit_canonical": {"variant": "reread_visit", "condition": "canonical"},
    "resolution_code_canonical": {"variant": "reread_code", "condition": "canonical"},
    "depth_state_s1": {"variant": "depth_state", "condition": "stability_1"},
    "depth_reread_s1": {"variant": "depth_reread", "condition": "stability_1"},
    "depth_state_s2": {"variant": "depth_state", "condition": "stability_2"},
    "depth_reread_s2": {"variant": "depth_reread", "condition": "stability_2"},
    "depth_state_s3": {"variant": "depth_state", "condition": "stability_3"},
    "depth_reread_s3": {"variant": "depth_reread", "condition": "stability_3"},
}
        ...
        for start in range(0, len(rows), BATCH_SIZE):
            batch = {
                ...
                for key, value in pack_rows(
                    _model_rows(rows[start : start + BATCH_SIZE]), dx_count, proc_count
                ).items()
            }
...
def _config(
    lane: str, variant: str, condition: str, numeric_policy: Mapping[str, Any]
) -> Dict[str, Any]:
    return {
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    if args.lane not in LANES:
        raise RuntimeError("unknown survivor lane")
    lane_spec = LANES[args.lane]
    variant = lane_spec["variant"]
    condition = lane_spec["condition"]
    ...
    config = _config(args.lane, variant, condition, numeric_policy)
    state: Dict[str, Any] = {
        "schema_version": 1,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the survivor screen")
    ...
    model = SurvivorModel(dx_count, proc_count, variant).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        dev_logits = _predict_logits(
            model, dev_rows, dev_targets, dx_count, proc_count, device
        )
        candidates: List[SelectionCandidate] = []
        candidate_results: List[Dict[str, Any]] = []
        for op_index, op in enumerate(OPERATING_POINTS):
            metrics = _surface(
                dev_rows, dev_targets, dev_logits, op, vocabulary, ddi
            )
            candidates.append(
                SelectionCandidate(epoch, op, metrics["jaccard"], op_index)
            )
        ...
        if (
            ...
            or _best_key(checkpoint_best) < _best_key(best_candidate)
        ):
        ...
        state["selected_checkpoint"] = (
            best_candidate.checkpoint if best_candidate else None
        )
    ...
    train_logits = _predict_logits(
        model, train_rows, train_targets, dx_count, proc_count, device
    )
