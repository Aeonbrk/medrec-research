#!/usr/bin/env python3
"""Run one 30-epoch lane of the final relational architecture search."""
...
LANES: Dict[str, Dict[str, str]] = {
    "foundation_code": {"family": "foundation", "variant": "resolution_code"},
    "summary_add": {"family": "relational", "variant": "summary_add"},
    "summary_mul": {"family": "relational", "variant": "summary_mul"},
    "factorized_pair": {"family": "relational", "variant": "factorized_pair"},
    "nonseparable_pair": {"family": "relational", "variant": "nonseparable_pair"},
    "joint_competition_pair": {
        "family": "relational",
        "variant": "joint_competition_pair",
    },
    "untyped_edge_pair": {"family": "relational", "variant": "untyped_edge_pair"},
    "temporal_edge_pair": {"family": "relational", "variant": "temporal_edge_pair"},
}
...
def _make_model(
    ...
    if family == "relational":
        return FinalRelationalModel(diagnosis_count, procedure_count, variant)
    raise RuntimeError("unknown architecture family: " + family)
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    if args.lane not in LANES:
        raise RuntimeError("unknown final architecture lane")
    ...
    config = _config(args.lane, family, variant, numeric_policy)
    state: Dict[str, Any] = {
        "schema_version": 1,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the full architecture screen")
    ...
    model = _make_model(dx_count, proc_count, family, variant).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if best_candidate is None or _best_key(checkpoint_best) < _best_key(
            best_candidate
        ):
    ...
    horizon_censored = best_candidate.checkpoint > HORIZON_SAFE_MAX_EPOCH
    result: Dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
        ...
        "epoch_30_Dev": evaluations[-1]["dev_metrics"],
    ...
    state["epoch_30_Dev"] = evaluations[-1]["dev_metrics"]
    _write_json(output / "progress.json", state)
