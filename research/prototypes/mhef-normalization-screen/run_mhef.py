#!/usr/bin/env python3
"""Run one 30-epoch lane of the MHEF normalization-domain screen."""
...
LANES: Dict[str, str] = {
    "mhef_independent_add": "mhef_independent_add",
    "coupled_budget_add": "coupled_budget_add",
    "wide_global_add": "wide_global_add",
    "mhef_independent_concat": "mhef_independent_concat",
    "coupled_budget_concat": "coupled_budget_concat",
    "private_only_add": "private_only_add",
    "hash_partition_a_add": "hash_partition_a_add",
    "hash_partition_b_add": "hash_partition_b_add",
}
...
def _make_model(diagnosis_count: int, procedure_count: int, variant: str) -> MHEFModel:
    return MHEFModel(diagnosis_count, procedure_count, variant)
...
def _config(
    lane: str, variant: str, numeric_policy: Mapping[str, Any], epochs: int
) -> Dict[str, Any]:
    return {
        "profile_id": PROFILE_ID,
        "architecture_family": "MHEF",
        "lane": lane,
        "variant": variant,
        "epochs": epochs,
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    if args.lane not in LANES:
        raise RuntimeError("unknown MHEF lane")
    variant = LANES[args.lane]
    if variant not in MHEF_VARIANTS:
        raise RuntimeError("lane maps to an unknown MHEF variant")
    ...
    if epochs not in {30, 60}:
        raise RuntimeError("MHEF runner supports only the frozen 30-epoch screen or its 60-epoch censoring extension")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the full MHEF screen")
    ...
    horizon_censored = (
        ...
        and best_candidate.checkpoint > HORIZON_SAFE_MAX_EPOCH
    )
    result: Dict[str, Any] = {
        "schema_version": 1,
        "status": "complete",
