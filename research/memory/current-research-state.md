...
- promote SSPNet when structured/set prediction becomes the active hypothesis, SSPNet is confirmed as the closest relevant comparator, and a trustworthy execution path exists. A bounded source probe reached a finite forward only after isolating plotting, then failed in the official empty-set Jaccard evaluator; this remains non-evidence and requires an independently reviewed adapter.
