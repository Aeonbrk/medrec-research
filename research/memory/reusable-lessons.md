...
Establishing that an oracle routing or revision allocation has substantial headroom over trivial baselines (e.g. random or risk-only) proves that heterogeneity exists, but does not validate any specific proposed selection mechanism. When a strong observable control (such as the base recommender's own confidence) explains substantial variance, the proposed additional signal must demonstrate preregistered incremental information beyond that control before mechanism-specific modeling or architectural expansion is justified. Evidence: Idea 001 Gate 01 (`gate-summary.json`) vs Gate 02 (`gate-02-summary.json`), Idea 002 Gate 01 (`gate-01-summary.json`), and Idea 003 Gate 01 (`gate-01-summary.json`).

## Conditional feature failure does not exhaust an information family

When a preregistered candidate feature fails to improve over a strong control on held-out evidence, terminate the tested feature, representation, and same-information cosmetic variants. Do not infer that every observable in the broader information family is useless or that the control explains all available variance. A successor route must introduce materially different observable information or a different scientific formalization, then face its own closest-work search, strongest simple control, and preregistered incremental test. Evidence: Ideas 001--003 and `research/memory/failures/prescription-relative-confidence-gate-01--no-incremental-relative-confidence.md`.
